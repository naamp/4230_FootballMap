...
def get_coordinates(city, stadium_name):
    url = f'https://nominatim.openstreetmap.org/search.php?q={stadium_name}+{city}&format=json'
    response = requests.get(url)
    if response.status_code == 200:
        data = response.json()
        stadium_entry = None
